...
def save_qr_data_to_step_file(qr_data, output_file):
    ...
    data_64 = ''.join(data_parts)
    with open(output_file, "wb") as fileBinary:
        fileBinary.write(base64.b64decode(data_64))
...
def make_qr_ack_pres(metadata_qr):
    ...
    while True:
        ...
        cv2.imshow("Device_ATTAQUANT_Afficheur_QRCodes_ACK", qr_code_ack)
        # Positionnement délai
        cv2.waitKey(5)
        #  # Déplacer la fenêtre à un emplacement spécifique de l'écran
        cv2.moveWindow("Device_ATTAQUANT_Afficheur_QRCodes_ACK", 2200, 300)  # Coordonnées x=100, y=100
        # Positionnement délai
        cv2.waitKey(5)
        return
            ...
            if len(qrcodes_pyz):
                ...
                for qrcode in qrcodes_pyz:
                    ...
                    if qrcode.type == "QRCODE":  # sécurité contre les autres types que QRCODE (BARCODE)
                        ...
                        # cv2.imshow("Device_ATTAQUANT_Lecture_des_QRCodes_AFFICHES", frame)
                        #  Positionnement délai (milliSec)
                        # cv2.waitKey(10)
                        #  Déplacer la fenêtre à un emplacement spécifique de l'écran
                        # cv2.moveWindow("Device_ATTAQUANT_Lecture_des_QRCodes_AFFICHES", 0, 1500)  # Coordonnées x=100, y=100
                        #  Positionnement délai (milliSec)
                        # cv2.waitKey(10)
